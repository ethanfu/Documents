...
7. 还有一个就是在Mac下IntelliJ代码区域是支持手势放大的，这个在给别人演示的时候很是方便，但是Tomcat等Run的界面的时候默认能理解的基本就是需要手动的拖动大小很是不便，你可以尝试下`⌘+shift+up/down`,当然其他的project视图或者其他的也都可以利用这个快捷键配置上下左右的方向键来配置使用，方便的很，强烈推荐使用。
